...
//                        allCondition[4] = true;
                    ...
                    //信用记录 : 信用良好，无逾期
                    if(info.getC_name().equals("信用记录") && info.getC_value().equals("信用良好，无逾期")/*||info.getC_value().equals("信用良好，无逾期"))*/)
                        allCondition[5] = true;
            ...
            if((allCondition[0] || allCondition[3]  || allCondition[1] || allCondition[2])&& allCondition[5])
            {                //满足所有条件，自动买断
                new Handler().postDelayed(new Runnable(){
                    public void run() {
